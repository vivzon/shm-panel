...
<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row justify-between items-end gap-4 border-b border-white/5 pb-6">
        <div>
            ...
            <p class="text-slate-400">Welcome back, <span class="text-blue-400 font-bold">
                    <?= htmlspecialchars($username) ?>
                </span>. System is running
                smoothly.</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="glass-card p-6 relative overflow-hidden group hover:-translate-y-1 transition duration-300">
            ...
            <div class="flex justify-between items-start mb-4 relative z-10">
                ...
                <span class="text-xs font-bold bg-white/5 px-2 py-1 rounded text-slate-400">
                    <?= $usage_dom ?> /
                    <?= $clientData['max_domains'] ?>
                </span>
            </div>
            <h3 class="text-3xl font-bold text-white mb-1 relative z-10">
                <?= $usage_dom ?>
            </h3>
        ...
        <div class="glass-card p-6 relative overflow-hidden group hover:-translate-y-1 transition duration-300">
            ...
            <div class="flex justify-between items-start mb-4 relative z-10">
                <div class="p-3 bg-orange-500/10 text-orange-400 rounded-xl"><i data-lucide="hard-drive"
                        class="w-6 h-6"></i></div>
                <span
                    class="text-xs font-bold bg-orange-500/10 text-orange-400 border border-orange-500/20 px-2 py-1 rounded">
                    <?= htmlspecialchars($clientData['pkg_name']) ?>
                </span>
            </div>
            <h3 class="text-3xl font-bold text-white mb-1 relative z-10">
                <?= $disk_mb ?> MB
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex justify-between text-sm py-2 border-b border-white/5">
                        ...
                        <span class="font-mono text-emerald-400">
                            <?= isset($_SERVER['SERVER_SOFTWARE']) ? explode('/', $_SERVER['SERVER_SOFTWARE'])[0] : 'Unknown' ?>
                        </span>
                    </div>
                    <div class="mt-4 pt-2">
...
<script>
    ...
    async function fetchLogs() {
        try {
            // Updated to point to new API endpoint
    ...
    async function clearLogs() {
        ...
        try {
            ...
            fd.append('token', '<?= $_SESSION['csrf_token'] ?>');
            await fetch('api/stats.php', { method: 'POST', body: fd });
            fetchLogs();
        } catch (e) { console.error(e); }
    }
