...
<div class="space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="glass-card p-6 border-blue-500/20">
            <div class="flex items-center gap-4 mb-4">
                ...
                <div>
                    <h3 class="font-bold text-white text-lg">Total Hits</h3>
                    <p class="text-xs text-slate-400 font-bold uppercase tracking-wider">All Time</p>
                </div>
            </div>
            <div class="text-3xl font-bold font-heading text-white">
        ...
        <div class="glass-card p-6 border-purple-500/20">
            <div class="flex items-center gap-4 mb-4">
                ...
                <div>
                    <h3 class="font-bold text-white text-lg">Total Bandwidth</h3>
                    <p class="text-xs text-slate-400 font-bold uppercase tracking-wider">All Time</p>
                </div>
            </div>
            <div class="text-3xl font-bold font-heading text-white">
    ...
    <div class="glass-card overflow-hidden text-sm">
        <div class="p-4 border-b border-slate-700/50 bg-slate-900/50 flex justify-between items-center">
            <h3 class="font-bold text-white">Domain Traffic</h3>
            <button onclick="location.reload()" class="text-slate-400 hover:text-white transition group">
                <i data-lucide="refresh-cw" class="w-4 h-4 group-hover:rotate-180 transition duration-500"></i>
            </button>
        </div>
        <table class="w-full text-left">
            <thead class="bg-slate-900/50 text-[10px] uppercase text-slate-500 font-bold tracking-wider">
                <tr>
            ...
            <tbody class="divide-y divide-slate-800">
                <?php if (empty($domains_traffic)): ?>
                    <tr>
                        <td colspan="3" class="p-8 text-center text-slate-500">No domains found</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($domains_traffic as $t): ?>
                        <tr class="hover:bg-slate-800/20 transition group">
                            <td class="p-5">
                                ...
                                <div class="text-[10px] text-slate-500 mt-1 uppercase tracking-wider font-bold">
                                    Last Activity:
