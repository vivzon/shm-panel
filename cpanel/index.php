...
<body class="bg-slate-50 flex h-screen overflow-hidden text-slate-800">
    ...
    <aside class="w-72 bg-slate-950 text-white flex flex-col p-6 shadow-2xl">
        <div class="text-2xl font-black text-blue-500 mb-10 flex items-center gap-2 italic"><i
                data-lucide="shield-check"></i> VIVZON</div>
        <nav class="flex-1 space-y-1">
            <button onclick="tab('dash')"
                class="nav-btn active w-full text-left p-4 rounded-2xl hover:bg-slate-800 flex gap-4 transition"><i
                    data-lucide="layout"></i> Dashboard</button>
            <button onclick="tab('files')"
                class="nav-btn w-full text-left p-4 rounded-2xl hover:bg-slate-800 flex gap-4 transition"><i
                    data-lucide="folder"></i> File Manager</button>
            <button onclick="tab('db')"
                class="nav-btn w-full text-left p-4 rounded-2xl hover:bg-slate-800 flex gap-4 transition"><i
                    data-lucide="database"></i> Databases</button>
            <button onclick="tab('mail')"
                class="nav-btn w-full text-left p-4 rounded-2xl hover:bg-slate-800 flex gap-4 transition"><i
                    data-lucide="mail"></i> Email Accounts</button>
            <button onclick="tab('dom')"
                class="nav-btn w-full text-left p-4 rounded-2xl hover:bg-slate-800 flex gap-4 transition"><i
                    data-lucide="globe"></i> DNS & PHP</button>
        </nav>
        <a href="logout.php" class="p-4 text-red-400 flex gap-3 font-bold hover:text-red-300 transition"><i
        ...
        <div id="pane-dash" class="pane active">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
                <div class="bg-white p-6 rounded-[2rem] border shadow-sm">
                    <div class="flex justify-between mb-4"><span
                            class="text-xs font-bold text-slate-400 uppercase">Databases</span><span
                            class="font-bold"><?= count($my_dbs) ?> / <?= $client['max_databases'] ?></span></div>
                    <div class="h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div class="bg-blue-600 h-full"
                            style="width:<?= (count($my_dbs) / $client['max_databases']) * 100 ?>%"></div>
                    </div>
                </div>
                <div class="bg-white p-6 rounded-[2rem] border shadow-sm">
                    <div class="flex justify-between mb-4"><span
                            class="text-xs font-bold text-slate-400 uppercase">Emails</span><span
                            class="font-bold"><?= count($my_emails) ?> / <?= $client['max_emails'] ?></span></div>
                    <div class="h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div class="bg-purple-600 h-full"
                            style="width:<?= (count($my_emails) / $client['max_emails']) * 100 ?>%"></div>
                    </div>
                </div>
                <div class="bg-white p-6 rounded-[2rem] border shadow-sm">
                    <div class="flex justify-between mb-4"><span
                            class="text-xs font-bold text-slate-400 uppercase">Domain Slots</span><span
                            class="font-bold"><?= count($domains) ?> / <?= $client['max_domains'] ?></span></div>
                    <div class="h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div class="bg-emerald-600 h-full"
                            style="width:<?= (count($domains) / $client['max_domains']) * 100 ?>%"></div>
                    </div>
                </div>
            </div>
            <?php
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                ...
                <a href="http://webmail.<?= $base_domain ?>" target="_blank"
                    ...
                    <p class="font-bold">Webmail</p>
                </a>
                <div onclick="tab('dom')"
                    class="bg-white p-8 rounded-3xl border text-center hover:shadow-xl transition group border-emerald-100 cursor-pointer">
                    <i data-lucide="shield-check"
                        class="mx-auto w-10 h-10 text-emerald-500 mb-3 group-hover:scale-110 transition"></i>
                    <p class="font-bold">SSL & DNS</p>
                </div>
            </div>
        </div>
                ...
                <div class="bg-white p-10 rounded-[2.5rem] border mb-8 shadow-sm">
                    ...
                    <div class="border-t pt-8">
                        ...
                        <form onsubmit="handle(event, 'add_dns')" class="grid grid-cols-4 gap-3 mb-4">
                            <input type="hidden" name="domain_id" value="<?= $d['id'] ?>">
                            <input name="host" placeholder="Host (e.g. @)" class="bg-slate-50 p-4 rounded-xl border text-sm"
                                required>
