...
function shm_normalize_relative($path) {
    ...
    $path = '/' . ltrim($path, '/');
    $parts = array_filter(explode('/', $path), strlen(trim($path)));
    $safe = [];
    foreach ($parts as $part) {
        if ($part === '.') continue;
        if ($part === '..') { array_pop($safe); } else { $safe[] = $part; }
    }
    return '/' . implode('/', $safe);
}
...
function shm_rrmdir($path) {
    ...
    foreach (scandir($path) as $item) {
        ...
        if (!shm_rrmdir($path . DIRECTORY_SEPARATOR . $item)) return false;
    }
    return @rmdir($path);
}
...
if (!$domain) {
    ...
    if($fid) { header("Location: ?domain_id=$fid&path=/"); exit; }
    die("No domains found. Please add a domain first.");
}

$base_path = rtrim($domain['document_root'] ?? "/home/".$_SESSION['client']."/public_html", '/');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['create_item'])) {
        ...
        if ($_POST['type'] == 'folder') mkdir($target, 0755);
        else file_put_contents($target, '');
        header("Location: ?domain_id=$domain_id&path=$current_path"); exit;
    }
    ...
    if (isset($_POST['zip_paths'])) {
        ...
        if ($zip->open($zip_name, ZipArchive::CREATE) === TRUE) {
            foreach ($_POST['paths'] as $p) {
                $abs = shm_build_path($base_path, $p);
                if (is_file($abs)) $zip->addFile($abs, basename($abs));
            }
            $zip->close();
        }
        header("Location: ?domain_id=$domain_id&path=$current_path"); exit;
...
<head>
    ...
    <style>
        ...
        .file-row:hover { background: #f1f5f9; cursor: pointer; }
        .file-row.selected { background: #e0f2fe; border-left: 4px solid #0ea5e9; }
        .context-menu { position: fixed; background: white; border: 1px solid #e2e8f0; box-shadow: 0 10px 15px -3px rgb(0 0 0 / 0.1); border-radius: 8px; z-index: 100; min-width: 180px; display: none; }
        #selection-toolbar { position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); background: #0f172a; color: white; padding: 12px 24px; border-radius: 50px; display: none; align-items: center; gap: 20px; box-shadow: 0 20px 25px -5px rgb(0 0 0 / 0.2); z-index: 1000; }
    </style>
</head>
<body class="flex flex-col h-screen">
    ...
    <header class="bg-white border-b px-8 py-4 flex items-center justify-between">
        <div class="flex items-center gap-6">
            <h1 class="text-xl font-bold text-slate-900 flex items-center gap-2"><i data-lucide="folder-tree" class="text-blue-600"></i> File Manager</h1>
            <div class="flex items-center text-sm text-slate-400">
                <a href="?domain_id=<?=$domain_id?>&path=/" class="hover:text-blue-600">Root</a>
                <?php 
                $crumbs = explode('/', trim($current_path, '/'));
                $path_acc = '';
                foreach($crumbs as $c): if($c == '') continue; $path_acc .= '/'.$c; ?>
                <span class="mx-2">/</span><a href="?domain_id=<?=$domain_id?>&path=<?=$path_acc?>" class="hover:text-blue-600"><?=$c?></a>
                <?php endforeach; ?>
            </div>
        </div>
        <div class="flex gap-3">
            <button onclick="openModal('upload')" class="bg-blue-600 text-white px-4 py-2 rounded-lg font-bold flex gap-2"><i data-lucide="upload" class="w-4"></i> Upload</button>
            <button onclick="openModal('create')" class="bg-slate-800 text-white px-4 py-2 rounded-lg font-bold flex gap-2"><i data-lucide="plus" class="w-4"></i> New</button>
        </div>
    </header>
    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <aside class="w-64 bg-white border-r p-6 overflow-y-auto">
            ...
            <div class="space-y-1">
                ...
                <a href="?domain_id=<?=$d['id']?>" class="flex items-center gap-3 p-3 rounded-xl transition <?=$d['id']==$domain_id?'bg-blue-50 text-blue-600 font-bold':'hover:bg-slate-50'?>">
        ...
        <main class="flex-1 overflow-y-auto p-8" oncontextmenu="return false;">
            <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody id="file-body">
                        <?php if($current_path != '/'): ?>
                        <tr class="file-row border-b" onclick="window.location='?domain_id=<?=$domain_id?>&path=<?=dirname($current_path)?>'">
                            <td class="p-4"></td>
                            <td class="p-4 flex items-center gap-3"><i data-lucide="corner-left-up" class="w-4 text-slate-400"></i> ..</td>
                            <td colspan="3"></td>
                        </tr>
                        <?php endif; ?>

                        <?php foreach($items as $i): ?>
                        <tr class="file-row border-b group" 
                            data-path="<?=$i['rel']?>" 
                            ...
                            <td class="p-4 text-center"><input type="checkbox" class="file-check" value="<?=$i['rel']?>"></td>
                            <td class="p-4 flex items-center gap-3 font-medium">
                                <i data-lucide="<?=$i['is_dir']?'folder':'file-text'?>" class="w-5 <?=$i['is_dir']?'text-amber-400':'text-blue-400'?>"></i>
                                <?=$i['name']?>
                            </td>
    ...
    <div id="selection-toolbar">
        ...
        <div class="flex gap-4">
            <button onclick="bulkAction('delete')" class="flex gap-2 text-red-400 hover:text-red-300"><i data-lucide="trash-2"></i> Delete</button>
            <button onclick="bulkAction('zip')" class="flex gap-2 text-blue-400 hover:text-blue-300"><i data-lucide="file-archive"></i> Zip</button>
        </div>
    </div>
    ...
    <div id="modal-upload" class="fixed inset-0 bg-black/60 hidden flex items-center justify-center z-50">
        <div class="bg-white p-10 rounded-3xl w-full max-w-md">
            ...
            <div id="drop-zone" class="border-2 border-dashed border-slate-200 rounded-2xl p-10 text-center hover:border-blue-500 transition">
            ...
            <div id="upload-progress" class="mt-4 hidden">
                <div class="w-full bg-slate-100 h-2 rounded-full"><div id="progress-bar" class="bg-blue-600 h-full w-0 transition-all"></div></div>
            </div>
            <button onclick="closeModal('upload')" class="w-full mt-6 py-3 bg-slate-100 rounded-xl font-bold">Cancel</button>
        </div>
    </div>
