...
if (isset($_POST['ajax_action'])) {
    ...
    try {
        if ($_POST['ajax_action'] == 'save_config') {
            // Placeholder: Call system command to save configs or write to file
            echo json_encode(['status' => 'success', 'msg' => 'Configuration saved successfully!']);
            exit;
        }
        if ($_POST['ajax_action'] == 'restart_services') {
            // Placeholder: Call system to restart nginx and fpm
...
<div class="max-w-7xl mx-auto w-full flex flex-col lg:flex-row gap-8">
    ...
    <div class="flex-1 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            ...
            <div class="flex items-center gap-3">
                <div id="status-badge" class="flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-bold uppercase tracking-wider">
                    <div class="w-2 h-2 rounded-full bg-emerald-400"></div>
                    <span id="status-text">Running</span>
                </div>
                
                <button onclick="saveChanges()" class="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2 rounded-xl font-bold shadow-lg shadow-blue-500/20 transition flex items-center gap-2 text-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="glass-card p-6">
                ...
                <div class="mb-5 flex items-center justify-between">
                    ...
                    <div class="relative inline-block w-10 mr-2 align-middle select-none transition duration-200 ease-in">
                        <input type="checkbox" id="toggleUnlimited" onchange="toggleUnlimitedUploads()" class="toggle-checkbox absolute block w-5 h-5 rounded-full bg-white border-4 border-slate-700 appearance-none cursor-pointer transition-transform duration-200 ease-in-out z-10"/>
                        <label for="toggleUnlimited" class="toggle-label block overflow-hidden h-5 rounded-full bg-slate-700 cursor-pointer"></label>
                    </div>
                </div>

                <div class="space-y-4">
                    <div id="grp-client-max-body">
                        <label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-wide">Client Max Body Size <span class="text-slate-500 normal-case tracking-normal ml-1">client_max_body_size</span></label>
                        <div class="flex">
                            <input type="number" id="nginx_client_max_body_size" value="50" oninput="updatePreview()" class="w-full bg-slate-900/50 border border-slate-700 border-r-0 rounded-l-xl p-2.5 outline-none focus:border-blue-500 text-white text-sm transition">
                            <div class="bg-slate-800 border border-slate-700 border-l-0 rounded-r-xl px-4 flex items-center text-slate-400 text-sm font-medium">MB</div>
                        </div>
                    </div>
                    
                    <div>
                        <label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-wide">Client Body Timeout</label>
                        <div class="flex">
                            <input type="number" id="nginx_client_body_timeout" value="60" oninput="updatePreview()" class="w-full bg-slate-900/50 border border-slate-700 border-r-0 rounded-l-xl p-2.5 outline-none focus:border-blue-500 text-white text-sm transition">
                            <div class="bg-slate-800 border border-slate-700 border-l-0 rounded-r-xl px-4 flex items-center text-slate-400 text-sm font-medium">s</div>
                        </div>
                    </div>
                    
                    <div>
                        <label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-wide">Send Timeout</label>
                        <div class="flex">
                            <input type="number" id="nginx_send_timeout" value="60" oninput="updatePreview()" class="w-full bg-slate-900/50 border border-slate-700 border-r-0 rounded-l-xl p-2.5 outline-none focus:border-blue-500 text-white text-sm transition">
                            <div class="bg-slate-800 border border-slate-700 border-l-0 rounded-r-xl px-4 flex items-center text-slate-400 text-sm font-medium">s</div>
                        </div>
                    </div>
                </div>
            ...
            <div class="glass-card p-6">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-wide">FastCGI Read Timeout</label>
                        <div class="flex">
                            <input type="number" id="fcgi_read_timeout" value="60" oninput="updatePreview()" class="w-full bg-slate-900/50 border border-slate-700 border-r-0 rounded-l-xl p-2.5 outline-none focus:border-blue-500 text-white text-sm transition">
                            <div class="bg-slate-800 border border-slate-700 border-l-0 rounded-r-xl px-4 flex items-center text-slate-400 text-sm font-medium">s</div>
                        </div>
                    </div>
                    
                    <div>
                        <label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-wide">FastCGI Send Timeout</label>
                        <div class="flex">
                            <input type="number" id="fcgi_send_timeout" value="60" oninput="updatePreview()" class="w-full bg-slate-900/50 border border-slate-700 border-r-0 rounded-l-xl p-2.5 outline-none focus:border-blue-500 text-white text-sm transition">
                            <div class="bg-slate-800 border border-slate-700 border-l-0 rounded-r-xl px-4 flex items-center text-slate-400 text-sm font-medium">s</div>
                        </div>
                    </div>
                    
                    <div>
                        <label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-wide">Proxy Read Timeout</label>
                        <div class="flex">
                            <input type="number" id="proxy_read_timeout" value="60" oninput="updatePreview()" class="w-full bg-slate-900/50 border border-slate-700 border-r-0 rounded-l-xl p-2.5 outline-none focus:border-blue-500 text-white text-sm transition">
                            <div class="bg-slate-800 border border-slate-700 border-l-0 rounded-r-xl px-4 flex items-center text-slate-400 text-sm font-medium">s</div>
                        </div>
                    </div>
                </div>
            ...
            <div class="glass-card p-6 md:col-span-2">
                <div class="flex items-center gap-3 mb-6 border-b border-slate-700 pb-4">
                    ...
                    <div class="ml-auto">
                        <select id="php_version" onchange="updatePreview()" class="bg-slate-900/50 border border-slate-700 rounded-lg px-3 py-1.5 text-sm text-slate-300 outline-none focus:border-blue-500">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-4">
                        <div>
                            <label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-wide">Upload Max Filesize</label>
                            <div class="flex">
                                <input type="number" id="php_upload_max_filesize" value="50" oninput="updatePreview()" class="w-full bg-slate-900/50 border border-slate-700 border-r-0 rounded-l-xl p-2.5 outline-none focus:border-blue-500 text-white text-sm transition">
                                <div class="bg-slate-800 border border-slate-700 border-l-0 rounded-r-xl px-4 flex items-center text-slate-400 text-sm font-medium">M</div>
                            </div>
                        </div>
                        
                        <div>
                            <label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-wide">Post Max Size</label>
                            <div class="flex">
                                <input type="number" id="php_post_max_size" value="64" oninput="updatePreview()" class="w-full bg-slate-900/50 border border-slate-700 border-r-0 rounded-l-xl p-2.5 outline-none focus:border-blue-500 text-white text-sm transition">
                                <div class="bg-slate-800 border border-slate-700 border-l-0 rounded-r-xl px-4 flex items-center text-slate-400 text-sm font-medium">M</div>
                            </div>
                        </div>
                        
                        <div>
                            <label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-wide">Memory Limit</label>
                            <div class="flex">
                                <input type="number" id="php_memory_limit" value="256" oninput="updatePreview()" class="w-full bg-slate-900/50 border border-slate-700 border-r-0 rounded-l-xl p-2.5 outline-none focus:border-blue-500 text-white text-sm transition">
                                <div class="bg-slate-800 border border-slate-700 border-l-0 rounded-r-xl px-4 flex items-center text-slate-400 text-sm font-medium">M</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div>
                            <div class="flex justify-between mb-1">
                                <label class="text-xs font-bold text-slate-400 uppercase tracking-wide">Max Execution Time</label>
                                <span class="text-white text-sm font-fira bg-slate-800 px-2 py-0.5 rounded" id="php_max_execution_time_val">60s</span>
                            </div>
                            <input type="range" id="php_max_execution_time" min="30" max="600" step="30" value="60" oninput="updateRange('php_max_execution_time_val', this.value); updatePreview()">
                        </div>
                        
                        <div>
                            <div class="flex justify-between mb-1">
                                <label class="text-xs font-bold text-slate-400 uppercase tracking-wide">Max Input Time</label>
                                <span class="text-white text-sm font-fira bg-slate-800 px-2 py-0.5 rounded" id="php_max_input_time_val">60s</span>
                            </div>
                            <input type="range" id="php_max_input_time" min="60" max="600" step="60" value="60" oninput="updateRange('php_max_input_time_val', this.value); updatePreview()">
                        </div>
                        
                        <div>
                            <label class="block text-xs font-bold text-slate-400 mb-1 uppercase tracking-wide">Default Socket Timeout</label>
                            <div class="flex">
                                <input type="number" id="php_default_socket_timeout" value="60" oninput="updatePreview()" class="w-full bg-slate-900/50 border border-slate-700 border-r-0 rounded-l-xl p-2.5 outline-none focus:border-blue-500 text-white text-sm transition">
                                <div class="bg-slate-800 border border-slate-700 border-l-0 rounded-r-xl px-4 flex items-center text-slate-400 text-sm font-medium">s</div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="glass-card p-6 md:col-span-2">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    <button onclick="applyPreset('default', this)" class="preset-btn p-4 rounded-xl border border-slate-700 bg-slate-800/30 hover:bg-slate-800 hover:border-blue-500 transition text-left group">
                    ...
                    <button onclick="applyPreset('large', this)" class="preset-btn p-4 rounded-xl border border-slate-700 bg-slate-800/30 hover:bg-slate-800 hover:border-blue-500 transition text-left group">
                    ...
                    <button onclick="applyPreset('backup', this)" class="preset-btn p-4 rounded-xl border border-slate-700 bg-slate-800/30 hover:bg-slate-800 hover:border-blue-500 transition text-left group">
                    ...
                    <button onclick="applyPreset('unlimited', this)" class="preset-btn p-4 rounded-xl border border-slate-700 bg-slate-800/30 hover:bg-slate-800 hover:border-blue-500 transition text-left group">
                ...
                <div class="flex flex-wrap gap-3 pt-6 border-t border-slate-700/50">
                    <button onclick="simulateAction('restart_services', 'Restarting Nginx & PHP-FPM...', true)" class="bg-indigo-600/20 text-indigo-400 border border-indigo-500/30 hover:bg-indigo-600 hover:text-white px-4 py-2 rounded-xl text-sm font-bold transition flex items-center gap-2">
                        <i data-lucide="refresh-cw" class="w-4 h-4"></i> Restart Services
                    </button>
                    <button onclick="simulateAction('test_config', 'Testing Configuration Syntax...', false)" class="bg-slate-800 text-slate-300 border border-slate-700 hover:bg-slate-700 hover:text-white px-4 py-2 rounded-xl text-sm font-bold transition flex items-center gap-2">
    ...
    <div class="w-full lg:w-96 flex flex-col gap-4">
        ...
        <div class="flex gap-2 p-1 bg-slate-900/80 rounded-xl border border-slate-800">
            <button onclick="switchPreview('nginx', this)" class="preview-tab active flex-1 py-2 rounded-lg text-sm font-bold transition bg-slate-800 text-white shadow-sm">
                nginx.conf
            </button>
            <button onclick="switchPreview('php', this)" class="preview-tab flex-1 py-2 rounded-lg text-sm font-bold text-slate-500 hover:text-slate-300 transition">
                php.ini
            </button>
        </div>
        ...
        <div class="glass-card flex-1 flex flex-col overflow-hidden min-h-[500px]">
            ...
            <div class="flex-1 bg-slate-950/80 p-4 overflow-y-auto custom-scrollbar">
                <pre id="preview-nginx" class="font-fira text-xs leading-relaxed text-slate-400 whitespace-pre-wrap word-break"></pre>
                <pre id="preview-php" class="font-fira text-xs leading-relaxed text-slate-400 whitespace-pre-wrap word-break hidden"></pre>
            </div>
        </div>
...
<span class="code-highlight">default_socket_timeout = ${inputs.php_default_socket_timeout.value}</span>`;
    ...
    function saveChanges() {
        simulateAction('save_config', 'Saving configuration changes...', false);
        setRestartWarning();
    }
</script>
